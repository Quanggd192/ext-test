...
export default function ExtendMenu({ hidden, onClose }: Props) {
  return (
    <div className='relative w-[32px]'>
      <div className={hidden ? "hidden" : "" + "absolute w-[280px] right-0 top-[-15px]"}>
        <div className='flex h-[25px]'>
          <div className='w-[90%]'></div>
          <IconClose onClick={onClose} className='w-[25px] h-[25px] cursor-pointer' />
        </div>
        <div className='text-[18px] bg-[#777]'>
          {listItems.map((item: any, index: number) => <a key={`extend-item-${index}`} className='block w-full h-[72px] leading-[72px] px-[20px] border-b border-solid border-[#888]' onClick={onClose} href={item.link}>{item.text}</a>)}
        </div>
      </div>
    </div>
  )
