...
const listChartFilterItems = ["日", "週", "月", "年"];
  ...
  return (
    <div className="w-[100%] bg-[#414141] max-h-[600px] px-[30px] py-[20px]">
      <div className="flex text-white font-[400] ">
        <div className="text-[15px] pr-[40px]">
          BODY
          <br /> RECORD
        </div>
      ...
      <div className="flex pb-[10px]">
        {listChartFilterItems.map((item: string, index: number) => (
          <div
            className="sm:mr-[20px] mr-[10px]"
            onClick={() => setActive(index)}
            key={`chart-filter-${index}`}
          >
            <ChartFilterItem isActive={index === active} text={item} />
          </div>
        ))}
      </div>
    </div>
